...
with tab1:
    ...
    if st.button("Calculate Cardiovascular Risk"):
        ...
        st.session_state['responses'] = ai_assistant_response("Cardiovascular", cardio_risk)
...
with tab2:
    ...
    hba1c = st.number_input("HbA1c (%)", min_value=4.0, max_value=14.0, value=5.5, key="hba1c")
    ...
    if st.button("Calculate Diabetes Risk"):
        ...
        st.session_state['responses'] = ai_assistant_response("Diabetes", diabetes_risk)
...
with tab3:
    ...
    fev1 = st.number_input("FEV1 (%)", min_value=0, max_value=100, value=80, key="fev1")
    ...
    if st.button("Calculate COPD Risk"):
        ...
        st.session_state['responses'] = ai_assistant_response("COPD", copd_risk)

# Asthma Risk Tab
with tab4:
    st.header("Asthma Risk Assessment")
    frequency_of_symptoms = st.number_input("Frequency of Symptoms (per week)", min_value=0, max_value=14, value=2, key="frequency_of_symptoms")
    nighttime_symptoms = st.number_input("Nighttime Symptoms (per month)", min_value=0, max_value=30, value=1, key="nighttime_symptoms")
    inhaler_use = st.number_input("Inhaler Use (per week)", min_value=0, max_value=14, value=2, key="inhaler_use")
    eosinophil_count = st.number_input("Eosinophil Count (cells/μL)", min_value=0, max_value=2000, value=300, key="eosinophil_count")
    ...
    if st.button("Calculate Asthma Risk"):
        ...
        st.session_state['responses'] = ai_assistant_response("Asthma", asthma_risk)
...
with tab5:
    ...
    if st.session_state['results']:
        ...
        st.write("Please complete the assessments in the other tabs to generate a care plan.")

# Display AI Assistant Recommendations
